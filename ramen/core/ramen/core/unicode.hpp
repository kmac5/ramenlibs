// Copied and adapted from Adobe source libraries.
...
#ifndef RAMEN_CORE_UNICODE_HPP
#define RAMEN_CORE_UNICODE_HPP
...
template<typename T>
struct is_utf8_type { enum { value = sizeof(T) == 1};};

template<typename T>
struct is_utf16_type { enum { value = sizeof(T) == 2};};

template<typename T>
struct is_utf32_type { enum { value = sizeof(T) == 4};};

template<typename I>
struct is_utf8_iterator_type { enum { value = is_utf8_type<typename std::iterator_traits<I>::value_type>::value};};
...
struct promotion_engine_t
{
    ...
    inline boost::uint32_t operator () (InputIterator& first, InputIterator last)
    {
        ...
        */
        ...
        if( first == last)
            throw runtime_error( "unicode: utf32 conversion ran out of input");
    ...
    to_utf32 (InputIterator first, InputIterator last, DestInteger& result)
{
        ...
        code <= implementation::utf16_high_surrogate_back_k)
    {
        ...
        if( first == last)
            throw runtime_error( "unicode: utf16 high surrogate found without low surrogate");
        ...
        { throw runtime_error( "unicode: utf16 low surrogate found without high surrogate");}
    else
    ...
    to_utf32 (InputIterator first, InputIterator last, DestInteger& result)
{
    ...
    if( n < implementation::to_utf32_pivot_1_k)
        ...
        { throw runtime_error( "unicode: ill-defined utf8 (< 192)");}
        ...
        { throw runtime_error( "unicode: ill-defined utf8 (>= 254)");}
